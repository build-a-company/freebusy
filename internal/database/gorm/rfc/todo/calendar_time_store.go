...
func (s *CalendarTimeStore) Create(ctx context.Context, m *CalendarTime) error {
	...
	start := time.Now()
	err := tel.Span(ctx, "todo.CalendarTime/Create", m, func(ctx context.Context) error {
		return s.DB.WithContext(ctx).Create(m).Error
	})
	tel.RecordOp(ctx, "todo.calendar_times", "create", time.Since(start), err)
	return err
}
...
func (s *CalendarTimeStore) List(ctx context.Context, opts gormx.ListOptions) ([]CalendarTime, error) {
	...
	tel := gormx.OrNop(s.Telemetry)
	start := time.Now()
	err := tel.Span(ctx, "todo.CalendarTime/List", nil, func(ctx context.Context) error {
		return opts.Apply(s.DB.WithContext(ctx)).Find(&out).Error
	})
	tel.RecordOp(ctx, "todo.calendar_times", "list", time.Since(start), err)
	if err != nil {
		return nil, err
	}
	return out, nil
...
func (s *CalendarTimeStore) Count(ctx context.Context, opts gormx.ListOptions) (int64, error) {
	...
	tel := gormx.OrNop(s.Telemetry)
	start := time.Now()
	err := tel.Span(ctx, "todo.CalendarTime/Count", nil, func(ctx context.Context) error {
	...
	tel.RecordOp(ctx, "todo.calendar_times", "count", time.Since(start), err)
	if err != nil {
		return 0, err
	}
	return n, nil
...
func (s *CalendarTimeStore) Update(ctx context.Context, m *CalendarTime) error {
	tel := gormx.OrNop(s.Telemetry)
	start := time.Now()
	err := tel.Span(ctx, "todo.CalendarTime/Update", m, func(ctx context.Context) error {
		return s.DB.WithContext(ctx).Save(m).Error
	})
	tel.RecordOp(ctx, "todo.calendar_times", "update", time.Since(start), err)
	return err
}
...
func (s *CalendarTimeStore) GetByID(ctx context.Context, id string) (*CalendarTime, error) {
	...
	tel := gormx.OrNop(s.Telemetry)
	start := time.Now()
	err := tel.Span(ctx, "todo.CalendarTime/GetByID", nil, func(ctx context.Context) error {
		return s.DB.WithContext(ctx).First(&m, "id = ?", id).Error
	})
	tel.RecordOp(ctx, "todo.calendar_times", "get", time.Since(start), err)
	if err != nil {
		return nil, err
	}
	return &m, nil
...
func (s *CalendarTimeStore) DeleteByID(ctx context.Context, id string) error {
	tel := gormx.OrNop(s.Telemetry)
	start := time.Now()
	err := tel.Span(ctx, "todo.CalendarTime/DeleteByID", nil, func(ctx context.Context) error {
		return s.DB.WithContext(ctx).Delete(&CalendarTime{}, "id = ?", id).Error
	})
	tel.RecordOp(ctx, "todo.calendar_times", "delete", time.Since(start), err)
	return err
}
