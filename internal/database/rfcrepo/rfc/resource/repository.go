...
type ResourceRepository interface {
	...
	Create(ctx context.Context, m *resourcev1.Resource) (*resourcev1.Resource, error)
	// Get returns the record addressed by its resource name, or
	// repox.ErrNotFound.
	Get(ctx context.Context, name string) (*resourcev1.Resource, error)
	// List returns one page of records.
	List(ctx context.Context, in repox.ListInput) ([]*resourcev1.Resource, string, error)
	...
	// (repox.ErrConflict).
	Update(ctx context.Context, m *resourcev1.Resource, paths []string) (*resourcev1.Resource, error)
...
type ResourceHooks struct {
	...
	BeforeCreate func(ctx context.Context, m *resourcev1.Resource) error
	// AfterRead runs on every record a read returns (Get, List, and the
	// re-reads writes return).
	AfterRead func(ctx context.Context, m *resourcev1.Resource) error
	// BeforeUpdate runs after the mask is applied to the merged record,
	// before it is written.
	BeforeUpdate func(ctx context.Context, existing, merged *resourcev1.Resource, paths []string) error
